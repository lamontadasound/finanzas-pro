...
import type { Ingreso, Gasto, Suplido, GastoEvento } from '../types';
...
export const DJPersonal = () => {
  const { ingresos, gastos, suplidos, gastosEvento, addIngreso, updateIngreso, deleteIngreso, addGasto, updateGasto, deleteGasto, addSuplido, updateSuplido, deleteSuplido } = useStore();
  ...
  const djGastosEvento  = useMemo(() => gastosEvento.filter((g) => g.area === 'dj'), [gastosEvento]);
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      {tab === 'resumen' && <ResumenTab ingresos={djIngresos} gastos={djGastos} gastosEvento={djGastosEvento} />}
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-surface-800 border border-surface-400/20 rounded-xl px-4 py-3">
          ...
          <p className="text-lg font-bold text-purple-400">{fmt(totIng)}</p>
        </div>
        <div className="bg-surface-800 border border-surface-400/20 rounded-xl px-4 py-3">
      ...
      <div className="bg-surface-800 border border-surface-400/20 rounded-xl overflow-hidden">
        ...
        {monthlyEnriched.filter((r) => r.ingMes.length > 0).length === 0 ? (
